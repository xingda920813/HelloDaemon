        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    List<PackageInfo> installedPackages = KillActivity.this.getPackageManager().getInstalledPackages(0);
                    for (PackageInfo pi : installedPackages)
                        if ((pi.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0 &&
                                (pi.applicationInfo.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) == 0) packageNames.add(pi.packageName);
                    Object am = KillActivity.this.getSystemService(ACTIVITY_SERVICE);
